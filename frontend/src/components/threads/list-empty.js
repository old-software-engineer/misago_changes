  ...
  render () {
    if (this.props.list.type === 'all') {
          ...
          <li className="list-group-item empty-message">
            <p className="lead">
              {this.props.category.special_role
                ? gettext("There are no questions on this forum... yet!")
                : gettext("There are no questions in this category.")}
            </p>
            <p>
              {gettext("Why not start one yourself?")}
            </p>
      ...
      return <li className="list-group-item empty-message">
        {gettext("No questions matching specified criteria were found.")}
      </li>;
      /* jshint ignore:end */
    }
  }
